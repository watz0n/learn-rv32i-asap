...
#endif
#include "Vrvtop.h" // chisel-generated code...
#include "verilator.h"
#include <fesvr/dtm.h>
#include "verilated.h"
#include <fcntl.h>
...
int main(int argc, char** argv)
{
   ...
   Vrvtop dut; // design under test, aka, your chisel code
  ...
  //fprintf(stderr, "Reset Done.\n");
   ...
   while (!dtm->done() && !dut.io_success && !Verilated::gotFinish()) {
      ...
      trace_count++;
      //printf("tc: %d\n", trace_count);  //Check if too close to end after load instructions to mem
                                          //Far away to end with wait nop
      if (max_cycles != 0 && trace_count == max_cycles)
      {
         //printf("dtmd:%d, dut.succ:%d, veri_gf:%d\n", dtm->done(), dut.io_success, Verilated::gotFinish());
         failure = "timeout";
         break;
      }
   }
